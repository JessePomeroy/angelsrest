# Pre-AR-04 architecture record — 2026-09-03

> Historical migration evidence only. This is the exact detailed architecture
> record immediately before the AR-04 documentation reset. Use
> [`../ARCHITECTURE.md`](../ARCHITECTURE.md) for current system boundaries and
> [`../contracts/architecture-invariants.md`](../contracts/architecture-invariants.md)
> for the stable safety contract.
...
9. `createAdminBrowserCapabilities` adapts the generated Convex proxy to an
   exact browser-safe Admin interface that forbids the server-owned
   document-email journal. `createAdminServerCapabilities` lives in a `.server`
   module and adds that journal for authenticated handler factories. Host config
   does not proxy or cast the generated API.
