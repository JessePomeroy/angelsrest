...
Once guided records exist, rollback must preserve their backend producer fences and recovery data. The backend returns a backward-compatible guided marker on its existing unavailable refund claim, so the current host still records supplier review with the flag off. Turning the host flag off stops new guided execution; it does not authorize old automatic refunds to take over affected orders. Resolve those orders with operator review before removing this contract.
