...
const detail = /\/admin\/editor\/(products|blog)\/.+/.test(params.get("route") ?? "") || params.get("state") === "populated";
...
function fixtureQuery(name: string, args: Args = {}) {
	...
	switch (name) {
		...
		case "crm:getStats": return { total: empty ? 0 : 3, leads: empty ? 0 : 1, booked: empty ? 0 : 1, inProgress: empty ? 0 : 1, completed: 0, photography: empty ? 0 : 2, web: empty ? 0 : 1 };
		case "invoices:list": return list(data.invoices);
		case "quotes:list": return list(data.quotes);
		case "contracts:list": return list(data.contracts);
		case "invoices:getNextNumber": return "INV-DEMO-004";
		case "quotes:getNextNumber": return "QUO-DEMO-004";
		case "emailTemplates:list": return list(data.emailTemplates);
		case "contracts:listTemplates": return list([{ _id: "demo-template-1", name: "Portrait agreement", body: "Demonstration agreement only.", variables: [] }]);
		case "quotes:listPresets": return [];
		...
		case "catalogProductGraphs:getEditorState": return data.product;
		case "postContent:listForEditor": return detail ? list(data.postSummaries) : [];
		case "postContent:getEditorState": return data.post;
		case "blogContent:listForEditor": return detail ? list(data.supportingSummaries.filter(document => !args.kind || document.kind === args.kind)) : [];
		case "blogContent:getEditorState": return data.supportingDocuments.find(document => document.documentId === args.documentId) ?? null;
		case "content:getSiteSettingsEditorState": return data.editorState(data.settingsPayload);
		case "content:getContactPageEditorState": return data.editorState(data.contactPayload);
		case "content:getAboutPageEditorState": return data.editorState({ heading: "About", displayName: "Demonstration artist", introduction: "A fictional biography for documenting the editor.", portraits: [], sections: [], highlights: [] });
		default: throw new Error(`No synthetic handbook query fixture for ${name}`);
	}
}
export function useQuery(ref: Ref, args: Args | (() => Args) = {}) {
	const result = $derived(loading ? undefined : fixtureQuery(getFunctionName(ref), typeof args === "function" ? args() : args));
	return { get data() { return result; }, get isLoading() { return loading; }, error: undefined };
}
export function usePaginatedQuery(ref: Ref, args: Args | (() => Args) = {}) {
	const result = $derived(fixtureQuery(getFunctionName(ref), typeof args === "function" ? args() : args));
	return {
		get results() { return result && typeof result === "object" && "page" in result ? result.page : []; },
		get status() { return loading ? "LoadingFirstPage" : "Exhausted"; },
...
export function useConvexClient() {
	return {
		...
		async mutation() { throw new Error("Simulated save failure: provider writes are disabled in handbook fixtures."); },
		async action() { throw new Error("Provider actions are disabled in handbook fixtures."); },
	};
}
...
export function useAuth() {
	return { isLoading: params.get("session") === "loading", isAuthenticated: params.get("session") !== "expired" };
}
