...
import type { PortalContractDocument } from "./portalPageData";

type Props = {
	document: PortalContractDocument;
	client: { name: string } | null;
	used: boolean;
	status: PortalContractDocument["status"];
...
<div class="doc-header">
	...
	<div class="doc-meta">
		...
		{#if doc.eventDate}<div class="meta-row"><span class="meta-label">Event date</span><span class="meta-value">{formatDateOnly(doc.eventDate)}</span></div>{/if}
...
{#if status === "sent" && !used}
	<div class="signature-section">
		...
		<div class="sig-field"><label for="signer-name">Your full name</label><input id="signer-name" type="text" bind:value={signerName} maxlength="200" placeholder="Enter your full name" /></div>
