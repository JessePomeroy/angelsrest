...
export async function load() {
	const [quotes, clients, nextNumber, presets, nextInvoiceNumber] =
		await Promise.all([
			...
			convex.query(api.invoices.getNextNumber, { siteUrl: SITE_DOMAIN }),
		]);
	return { quotes, clients, nextNumber, presets, nextInvoiceNumber };
}
