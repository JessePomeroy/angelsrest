...
- 8494a71: Enforce a global single-flight lease for CMS editor inspection dispatch claims while preserving fenced retries, tenant-scoped stale reconciliation and one-shot terminal outcomes. Legacy leases without expiry now fail closed across tenants until their owner repairs or terminalizes them.

## 2.27.0

### Minor Changes

- 805fb92: Add the tenant-scoped durable private catalog editor upload journal, capability vault, and fenced storage/inspection outbox APIs. Require exact Gallery Worker prepare-response attestation, globally separate Convex authority credentials and raw capabilities, preserve post-upload recovery across upload-token expiry, and durably retry scheduled Stripe fee capture while authority configuration is invalid.
