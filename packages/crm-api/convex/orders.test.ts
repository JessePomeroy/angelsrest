...
describe("order producer gate", () => {
	...
	test("keeps an authenticated existing-order replay read-only while closed", async () => {
		const t = convexTest(schema, modules);
		const args = orderArgs("cs_gate_replay");
		...
		const replay = await t.mutation(api.orders.create, args);

		expect(replay).toMatchObject({ _id: created._id, alreadyExisted: true });
