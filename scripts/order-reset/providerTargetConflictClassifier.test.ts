...
import {
	formatProviderTargetConflictClassification,
	parseProviderTargetConflictClassification,
	providerTargetClassifierChildEnvironment,
	providerTargetClassifierCliArguments,
...
describe("provider target-conflict classifier caller", () => {
	test("requires the exact host closed state, operation, and Convex target", () => {
		const ready = {
			ORDER_RESET_PROVIDER_TARGET_CLASSIFIER_ID: operationId,
			ORDER_PRODUCERS_STATE: "closed",
			PUBLIC_CONVEX_URL: convexUrl,
		};
		expect(providerTargetClassifierEnvironmentReady(ready, operationId, convexUrl)).toBe(true);
		for (const [name, value] of [
			["ORDER_RESET_PROVIDER_TARGET_CLASSIFIER_ID", undefined],
			["ORDER_RESET_PROVIDER_TARGET_CLASSIFIER_ID", "wrong"],
	...
	test("pins the internal function and deployment while sanitizing the child environment", async () => {
		expect(providerTargetClassifierCliArguments("loyal-swan-967")).toEqual([
			"run",
			"orderReset:classifyProviderTargetConflict",
			"{}",
			"--deployment",
			"loyal-swan-967",
			"--typecheck",
		...
		expect(providerTargetClassifierChildEnvironment(environment)).toEqual({
			HOME: "/home/operator",
			PATH: "/bin",
			XDG_CONFIG_HOME: "/config",
			XDG_CACHE_HOME: "/cache",
	...
	test("assembles one marker claim, host gate, internal read, and normalized result", async () => {
		...
		expect(result).toBe("provider_target_classifier:target_conflict:preparation_only");
	});
	...
	test("never reads when the marker or host gate fails and normalizes child failures", async () => {
		...
		).resolves.toBe("provider_target_classifier:operation_unavailable");
		...
		).resolves.toBe("provider_target_classifier:configuration_error");
		expect(readClassification).not.toHaveBeenCalled();
	...
	test("parses and formats only exact deterministic normalized classes", () => {
		for (const outcome of [
			...
			const parsed = parseProviderTargetConflictClassification(JSON.stringify({ outcome }));
			expect(formatProviderTargetConflictClassification(parsed)).toBe(
				`provider_target_classifier:${outcome}`,
			);
		}
		const parsed = parseProviderTargetConflictClassification(
			JSON.stringify({
		...
		expect(formatProviderTargetConflictClassification(parsed)).toBe(
			"provider_target_classifier:target_conflict:preparation_only+session_not_live",
		);
	});
	...
	test.each([
		...
		expect(() => parseProviderTargetConflictClassification(value)).toThrow("invalid");
	});
});
