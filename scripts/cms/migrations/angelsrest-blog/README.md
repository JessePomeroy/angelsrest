...
CMS-4.4o is read-only. It does not create Convex content, publish drafts, mutate Sanity or media, or change the public Blog provider. CMS-4.4p completed the bounded unpublished draft import after this gate passed.

## Retired CMS-4.4p writer

CMS-4.4p completed the bounded unpublished draft import. Its internal mutation and direct operator runner are retired and no longer callable. The fixed plan, store implementation, tests, mapping journal, and receipts remain versioned as historical custody and recovery evidence; they are not an executable migration interface.

The public Blog continues to read Sanity unless its separately controlled provider mode says otherwise. Do not recreate or invoke the retired import, reconciliation, or publication writers. Any future data repair needs a new reviewed interface with its own exact authority. Pinned Blog revision restore remains available through its separately bound internal restore mechanism.

## Bounded CMS-4.4n transfer

Preview the fixed one-asset batch without reading an admin Cookie or changing files or provider state:

```sh
pnpm cms:blog-media-transfer
```

Production execution is deliberately restricted to the sole still-unmapped published Blog image: the portrait on author `jesse` (`a149fe6f-cff8-400d-b4f2-c5b8b60bb815`) at `$.authors[0].image`:

```sh
pnpm cms:blog-media-transfer -- \
  --execute \
  --source-ref image-b97c89c82073b12c5b97b03ea3e1bdbd76d41e71-1832x3968-jpg \
  --cookie-file /absolute/path/to/owner-only-cookie.txt \
  --confirm "transfer CMS-4.4n 1-asset batch to www.angelsrest.online"
```
...
- processes the one reviewed asset and stops at the first failure;
...
- removes its machine-local checkpoint only after the asset passes final verification.
