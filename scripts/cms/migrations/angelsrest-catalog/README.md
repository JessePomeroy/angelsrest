...
All three files must be different regular, owner-controlled mode-`0600` files. Each file is opened once with no-follow semantics, then validated and read through that descriptor. The selector file may contain only `CONVEX_DEPLOYMENT=prod:loyal-swan-967`; it is not a deploy key. After validation, the runner writes that compiled-in exact selector to a fresh private temporary directory for each Convex child; this prevents the CLI from reopening the operator file or loading repository `.env` credentials, and it avoids the broader project-default `--prod` selector. The runner uses an existing non-interactive Convex operator login for only the bounded snapshot query and exact V1 authority backfill. It never performs OAuth, accepts object or target identities from the CLI, submits local inspection evidence, retries a failed network/Container operation automatically, or accesses R2 directly.

Every Worker request uses schema 2 and supplies one exact `expectation`: the receipt-set ID from the pinned Convex snapshot, deployment `prod:loyal-swan-967`, site origin `https://loyal-swan-967.convex.site`, full-raster policy `sharp_libvips_full_raster_v1` with Sharp `0.35.3` and libvips `8.18.3`, and ZIP policy `safe_zip_v1`. The runner rejects the old unattested response and requires an exact Worker-owned attestation before it continues: the route role, derived receipt identity, deployment, origin, decoder policy, and route-specific policy state must all match. Storage requires policy state `required`; inspection requires `matched`. This is a TLS-bound structured attestation, not a portable signature, and adds no secret. Against an older exact-parser Worker, the additional request object fails closed before receipt processing.

Only after a safe matching storage attestation does the runner observe the exact expected state transition on the pinned Convex deployment and permit the independently authenticated inspection call. Convex remains a final defense and independently rejects a different Angels Rest V2 identity or decoder version before recording evidence. Execution securely replaces its dedicated `${TMPDIR:-/tmp}/angelsrest-private-catalog-v2-canary-<uid>` directory with a fresh owner-controlled mode-`0700` directory before parsing or performing the run. A successful mode-`0600` `report.json` records its completion time, receipt identity, pinned deployment, labels, counts, decoder versions, and pass/fail checks, then removes any failure artifact.

A failed execution atomically creates or replaces `failure.json` with mode `0600` inside that private directory and preserves it for forensic resume. This bounded recovery signal contains only schema version, the last fixed execution phase, a fixed failure classification, an optional bounded HTTP status, and timestamp. It never persists selectors, object keys, receipt or target identities, URLs, hashes, credentials, response or error text, evidence, domains, or account identifiers. In particular, the `inspection` phase means the inspection call was attempted; an earlier `storage_replay` phase means it was not. Artifact-write failure exits nonzero with a fixed sanitized stderr message rather than being suppressed. The runner still performs no automatic network retry and resumes only through the existing exact server-state replay sequence. Production execution of this canary remains prohibited pending separate approval; this runner change does not perform a canary or any provider mutation.
